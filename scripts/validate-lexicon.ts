...
function loadLexiconData(filePath: string): LexiconWord[] {
  try {
    const content = readFileSync(filePath, 'utf-8')
    return JSON.parse(content) as LexiconWord[]
  } catch (error) {
...
function validateLexicon(lexicon: LexiconWord[]): ValidationResult {
  const errors: string[] = []
  ...
  const languageCompleteness: { [lang: string]: number } = {}
...
function main() {
  const lexiconPath = resolve(process.cwd(), 'src/entities/lexicon/data/master-lexicon.json')
